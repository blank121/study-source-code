;(function () {
  ...
  function FastClick (layer, options) {
    ...
    if (deviceIsAndroid) {
      layer.addEventListener('mouseover', this.onMouse, true);//true 捕获阶段触发
    ...
    if (!Event.prototype.stopImmediatePropagation) { // 阻止调用相同事件的其他侦听器。
  ...
  FastClick.prototype.onTouchStart = function (event) {
    ...
    if ((event.timeStamp - this.lastClickTime) < this.tapDelay) {//tapDelay默认200ms，两次点击在200ms内，第二次点无效
      event.preventDefault();//阻止300ms之后的原生click
    }
  ...
  FastClick.prototype.onTouchMove = function (event) {
    ...
    if (this.targetElement !== this.getTargetElementFromEventTarget(event.target) || this.touchHasMoved(event)) {//移动越界了，取消本次click加工处理，走原生流程
      this.trackingClick = false;
      this.targetElement = null;
    }
  ...
  FastClick.prototype.onTouchEnd = function (event) {
    ...
    //超时就不算click了，走原生流程，不阻止原生click也不模拟click
  ...
  FastClick.prototype.onMouse = function (event) {
    ...
    if (!this.needsClick(this.targetElement) || this.cancelNextClick) {
      ...
      //阻止targetElement上绑定的其他click handler
