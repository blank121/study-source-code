 ...
 */
//如果namespace为true,生成本地的dispatch,commit,getter和state
//const namespace = store._modules.getNamespace(path)
function makeLocalContext (store, namespace, path) {
  /* 判断是否有名字空间 */
  const noNamespace = namespace === ''

  //如果配置了modules，则简单包装一下，主要目的是构造path type
  const local = {
    dispatch: noNamespace ? store.dispatch : (_type, _payload, _options) => {
      ...
      //这里判断不是全局的情况，构造具体到module的type路径
  ...
  // 为local对象定义了getters和state属性的取值函数
  Object.defineProperties(local, {
    ...
    state: {
      get: () => getNestedState(store.state, path)//深度取值，并返回取到的值
    }
  })
...
function makeLocalGetters (store, namespace) {
  ...
  Object.keys(store.getters).forEach(type => {
    // type：完整路径（nameSpaced）
    // skip if the target getter is not match this namespace
    // slice提取一个字符串的一部分，并返回一新的字符串。
...
function registerMutation (store, type, handler, local) {
  ...
  entry.push(function wrappedMutationHandler (payload) {
    handler.call(store, local.state, payload)//绑定this为store，传入回调参数
  })
}
...
function getNestedState (state, path) {//深度取值，并返回取到的值
  return path.length
